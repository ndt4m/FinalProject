...
from PyQt6.QtWidgets import (
    ...
    QTabWidget,
    QFormLayout, 
    QStackedWidget, # Added for properties layout
)
...
class CMainWindow(MainWindowUI, MainWindowBase):
    def __init__(self, parent=None):
        ...
        self.category_colors = {
            ...
            "File System Manipulation": "#CCFFCC",     # Light green
            "Registry Manipulation": "#CCCCFF",      # Light blue
            "Process/Thread Manipulation": "#FFFFCC",  # Light yellow
        ...
        # Initialize log_api_widget as QStackedWidget
        self.log_api_widget = QStackedWidget()
        # Add a placeholder widget for when no process is selected
        placeholder_widget = QWidget()
        placeholder_layout = QVBoxLayout(placeholder_widget)
        placeholder_layout.addWidget(QLabel("No process selected or log file loaded."))
        placeholder_layout.addStretch()
        self.log_api_widget.addWidget(placeholder_widget)
    ...
    def start_monitoring_process(self, process_path, start_dir, runtime):
        monitor_exe = ".\\x64\\MonitoringApplication.exe"  # Update with actual path 
        dll_path = ".\\x64\\HookingDll.dll"  # Update with actual path
        ...
        try:
            ...
            process_info = {
                ...
                "log_view_widget": None,  # Will be set after parsing
                "summary_data": None  # Will be set after parsing
            }
            ...
            # Parse log file and create log view widget
            logs_by_category, summary_data = self.parse_log_file(process_info["log_file"])
            log_view_widget = self.create_log_view_widget(logs_by_category)
            self.log_api_widget.addWidget(log_view_widget)
            
            # Store widget and summary data in process_info
            process_info["log_view_widget"] = log_view_widget
            process_info["summary_data"] = summary_data
            
            # Display the log view and summary immediately
            self.log_api_widget.setCurrentWidget(log_view_widget)
            self.populate_summary_tabs(summary_data)
    ...
    def switch_process(self, item):
        ...
        for process_info in self.running_processes:
            if process_info["timestamp"] == timestamp:
                # Switch to the cached log view widget
                log_view_widget = process_info["log_view_widget"]
                self.log_api_widget.setCurrentWidget(log_view_widget)
                # Update summary tabs and properties
                self.populate_summary_tabs(process_info["summary_data"])
                self.display_properties(process_info["properties"])
                break

    def parse_log_file(self, log_file):
        """Parse the log file once and return categorized logs and summary data."""
        api_categories = {
            ...
            "File System Manipulation": ["ReadFile", "WriteFile", "DeleteFileW", "MoveFileW", "CopyFileExW"],
        ...
        try:
            ...
            pass
        return logs_by_category, summary_data

    def create_log_view_widget(self, logs_by_category):
        """Create a widget containing the log view for a process."""
        log_view_widget = QWidget()
        layout = QVBoxLayout(log_view_widget)
        for category, logs in logs_by_category.items():
            if logs:
                section = Section(category, animationDuration=100, parent=log_view_widget)
                table = QTableWidget()
    ...
    def populate_summary_tabs(self, summary_data):
        """Populate the summary tabs with pre-stored data."""
    ...
    def remove_process(self):
        ...
        selected_timestamp = selected_item.text().split(" - ")[0]
        for process_info in self.running_processes[:]:
            if process_info["timestamp"] == selected_timestamp:
                # Remove the log view widget from the stacked widget
                log_view_widget = process_info["log_view_widget"]
                if log_view_widget:
                    self.log_api_widget.removeWidget(log_view_widget)
                    log_view_widget.deleteLater()
